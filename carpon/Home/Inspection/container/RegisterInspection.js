...
import {resetProfile, settingTime} from "../actions/inspectionAction";
...
@screen('RegisterInspection', {header: <HeaderOnPress title='来店予約'/>})
@connect(state => ({
    ...
    dispatch => ({
        ...
        resetProfile: (data) => dispatch(resetProfile(data)),
    })
)
export class RegisterInspection extends Component {
    ...
    componentDidMount() {
        viewPage('submit_car_inspection_reservation_datetime', '車検予約日時選択');
    ...
    handleUpdate() {
        ...
        navigationService.navigate('ConfirmInspection', {store});
    }
    ...
    render() {
        ...
        return (
            <View contentInset={{ bottom: 25 }}
                  scrollIndicatorInsets={{right: 1}} style={{ flex: 1}}>
                <SingleColumnLayout
